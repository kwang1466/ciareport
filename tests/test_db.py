...
class TestDb(unittest.TestCase):
    ...
    def test_dbtool(self):
        db_tool = DBTool('webgoat', 'localhost', 2280)
        cursor = db_tool.get_schema('local')
        cursor.execute('select * from cdt_objects limit 100')
